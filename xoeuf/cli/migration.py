...
def _get_content(src, dst, file_name, fields):
    ...
    name = textwrap.fill(
        'Migrate (some) fields reference from %s to %s' % (
            src.module_name, src.module_name), 78)
    today = date.today()
    return MIGRATION_TEMPLATE.format(
        migration_name=file_name, current_year=today.year,
        today=Date.to_string(today), migration_readable_name=name,
        source_module=src.module_name, current_module=dst.module_name,
        ...
        for field in fields:
            ...
            if field and len(field) == 2:
                ...
                if (ref, tuple(field)) not in valid_fields:
                    raise ValueError('Given field (%s, %s) is not valid.' %
                                     field[-1])
    return result
...
class Migration(Command):
    ...
    def _get_arg_parser(cls):
        ...
        if not res:
            ...
            cls._arg_parser = res
            res.add_argument(
                '-d', '--database', dest='database', required=True, type=DB,
                help="Name of the database used used to check actual status.")
            res.add_argument(
                'src_module', type=Module,
                help="Name of the source module")
            res.add_argument(
                'dst_module', type=Module,
                help="Name of the destination module")
            res.add_argument(
    ...
    def run(self, args):
        ...
        if options.fields:
            fields = check_fields(options.fields, list(get_fields(
                options.database.db, options.src_module)))
        else:
            fields = []
            newline()
            write("Press y/n to select what fields include on migration. "
...
class Module(object):
    ...
    def get_file_name(self, src):
        ...
        base_name = 'pre-migrate_fields_reference_from_%s_to_%s' % (
            src.module_name, self.module_name)
        name = base_name
        count = 0
    ...
    def load_module(self, db):
        with db.db(transactional=True) as cr:
            ...
            if not module:
                raise ValueError(
                    'Not found module %s.' % self.module_name)
